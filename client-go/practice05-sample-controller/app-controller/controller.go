...
func (c *Controller) Run(workers int, stopCh <-chan struct{}) error {
	...
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.appsSynced, c.serviceSynced, c.configMapSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	configMap, err := c.configMapLister.ConfigMaps(app.Namespace).Get(app.Spec.ConfigMap.Name)
	if errors.IsNotFound(err) {
		configMap, err = c.kubeclientset.CoreV1().ConfigMaps(app.Namespace).Create(context.TODO(), newConfigMap(app), metav1.CreateOptions{})
	}
	...
	if !metav1.IsControlledBy(configMap, app) {
...
func newConfigMap(app *appv1alpha1.App) *corev1.ConfigMap {
